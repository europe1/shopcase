...
class Viewer {
  constructor(canvas, options) {
    ...
    switch (options.type) {
      case 'ORBIT':
        this.controls = new OrbitControls(this.defaultCamera, this.el);
        ...
        break;
      case 'EDITOR':
        this.controls = new TrackballControls(this.defaultCamera, this.el);
  ...
  addLights() {
    const areaLight = new THREE.RectAreaLight(0xffffff, 1.3, 3, 4);
    areaLight.name = 'areaLight';
    areaLight.position.set(0, 0, 1);
    ...
    var rectLightHelper = new THREE.RectAreaLightHelper(areaLight);
    areaLight.add(rectLightHelper);

    this.defaultCamera.add(areaLight);
  ...
  getCubeMapTexture(path, format) {
    ...
    if (format === '.hdr') {
      return new Promise((resolve) => {
        new HDRCubeTextureLoader().load( THREE.UnsignedByteType, cubeMapURLs, ( hdrCubeMap ) => {
